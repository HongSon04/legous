...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (isset($_SESSION['checkoutProduct']) && is_array($_SESSION['checkoutProduct'])) {
        ...
        $productQty = $newProductModel->getProductQtyById($id_product);
        $newQty = $productQty - $qty;

        /** update product purchases */
        $purchases = rand(10, 100);
        $newShippingModel = new shipping();
        $shipping = $newShippingModel->getShippingMethodByPrice($id_shipping)['price'];
        ...
        if (isset($_SESSION['userLogin'])) {
            ...
            if (empty($name_recipient)) {
                ...
                $name_recipient = $newUserModel->getUserInfo($id_user)[0]['username'];
            }
            $newBillModel = new bill();
            $idBill = $newBillModel->insertBill($id_user, $id_shipping, $id_payment, $email_user, $phone_user, $address_user, $address_detail_user, $name_recipient, $phone_recipient, $address_recipient, $address_detail_recipient, $total, 1);
            $_SESSION['idBill'] = $idBill;
            /** lấy id bill vừa tạo, insert sản phẩm muốn mua ngay vào bảng giỏ hàng */
            $newCartModel = new cart();
            $newCartModel->insertCartWithIdBill($idBill, $id_user, $id_product, $name, $price, $img, $qty, $subTotal);
            /** update số lượng của sản phẩm vừa mua */
            $newProductModel = new product();
            $newProductModel->updateProductQty($id_product, $newQty);
