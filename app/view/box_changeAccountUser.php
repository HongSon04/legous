...
foreach ($_COOKIE as $name => $value) {
    if (str_starts_with($name, "accounts_user")) {
        ...
        $accounts_user = $newUserModel->get_accountUser($value);
...
if (is_array($userList) && $userList != "") {
    foreach ($userList as $item) {
        foreach ($item as $key) {
            extract($key);
            if ($img == "") {
                $avatar_account = "./public/upload/users/" . 'avatar-none.png';
            } else {
                $avatar_account = "./public/upload/users/" . $img;
            }
...
<div class="box__changeAccount">
    <svg viewBox="0 0 20 20" fill="currentColor" class="x1lliihq x1k90msu x2h7rmj x1qfuztq xfuq9xy x10w6t97 x1td3qas">
    ...
    <div class="box__changeAccount--content box-shadow4">
        ...
        <div class="box__changeAccount--logOut">
            <a href="/logout" class="box__changeAccount--logOut__button" style="color: black;">
