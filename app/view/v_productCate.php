...
$maxPrice = $newProductModel->getMaxProductPrice();
$minPrice = $newProductModel->getMinProductPrice();
...
if (isset($_GET['idCategory'])) {
    ...
    $category = $newCategoryModel->getCategoryById($idCategory);
    $newProductModel = new Product();
    $featureProducts = $newProductModel->getFeatureProductOfCategory($idCategory, $limit = 0);
    $categoryProducts = $newProductModel->getProductsByCategoryId($idCategory);
    ...
    if (isset($_POST['priceFilter']) && $_POST['priceFilter']) {
        ...
        $max = $_POST['maxPrice'];
        $newProductModel = new Product();
        $categoryProducts = $newProductModel->getCategoryProductsByPriceFilter($min, $max, $idCategory);
    }
    ...
    if (isset($_GET['filterName']) and $_GET['filterName'] != '') {
        ...
        $newProductModel = new Product();
        $categoryProducts = $newProductModel->getProductsByCategoryId($idCategory, $order);
    }
}
?>
...
<script>
    ...
    $(document).ready(function () {
        $('.love-btn').click(function () {
            ...
            $.ajax({
                url: './app/library/update_love.php', // Replace with the correct path to your PHP script
...
foreach ($featureProducts as $item) {
    ...
    $featureProductsHtml .=
        <<<HTML
            <a href="/productDetail?idProduct=$id" class="feature-product feature-product__carousel flex g12 p20 rounded-8">
...
$newProductModel = new Product();
$categoryProducts = $newProductModel->getCategoryProductWithLimitAndOffset($productsPerPage, $offset, $idCategory);

// Fetch total number of products from the database
$totalProducts = count($newProductModel->getProductsByCategoryId($idCategory));
...
for ($i = 1; $i <= $totalPages; $i++) {
    $linkToPage = "/productCate?idCategory=$idCategory&page=$i";
    $active = '';
    if ($page == $i) {
        $active = 'active';
    }
...
if ($nextPage >= $totalPages) {
    ...
    $linkToNextPage = "/productCate?idCategory=$idCategory&page=$nextPage";
}
...
$newCategoryModel = new Category();
$categories = $newCategoryModel->getCategory();
foreach ($categories as $item) {
    extract($item);
    $newProductModel = new Product();
    $amount = count($newProductModel->getProductsByCategoryId($id));
    $categoryHtml .=
        <<<HTML
...
<main class="section shop__main">
            ...
            <li class="filter__list__item por">
                ...
                <ul class="filter-option__list poa p20 rounded-8 box-shadow1" style="right: 0">
                    <li class="filter-option__item por">
                        <form action="/productCate?idCategory=<?= $idCategory ?>" method="post">
                            <!-- normal form group -->
                            <div class="flex g20">
    ...
    <div class="filter-toggle__list--mobile mobile flex g20 flex-full"
        style="display: flex !important; margin-top: 1.2rem"></div>
