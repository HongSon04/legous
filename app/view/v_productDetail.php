...
if (isset($_GET['idProduct']) && $_GET['idProduct'] != 0) {
    ...
    $productThumbnails = $images->getThumbnailsById($idProduct);
    // $view = rand(10, 100);
    // $product = new product();

    // $product->updateProductViewById($idProduct, $view);
    ...
    $idCategory = intval($category->getIdCategoryByIdProducts($idProduct));
    $relatedProducts = $product->getRelatedProduct($idCategory, 12);
    $randomProducts = $product->getProduct(12);
    ...
    /** comment render */
    $comment = new comment();
    $comments = $comment->getProductCommentByProductId($idProduct);
    $commentHtml = '';
            ...
            <<<HTML
                    ...
                    <div class="comment__item mb30">
                        <div class="flex g12 comment__user">
                            <div class="user__avt avt"><img src="/public/images/users/user-1.svg"
...
foreach ($productThumbnails as $item) {
    extract($item);
    $imgPath = "./public/images/product/" . $src;
    $galleryThumbnailsHtml .=
        <<<HTML
...
<div class="mobile-top__bar mobile-top__bar--shop p12 flex-center">
    <div class="mobile-top__inner flex-full flex-between v-center g12">
        ...
        <a href="/viewCart" class="icon-btn cart-btn"><i class="fal fa-shopping-cart"></i></a>
...
<main class="section single-product__main auto-grid g30 mt80" style="display: grid !important">
    <div class="product__gallery mobile">
        <div class="gallery__spotlight">
            <img src="./public/images/product/<?= $img ?>" alt="" class="img-cover">
        </div>
    ...
    <div class="product__gallery por desktop" style="place-items: center">
        <div class="gallery__spotlight flex-center">
            <img src="./public/images/product/<?= $img ?>" alt="" class="img-contain">
        </div>
        ...
        <div class="panels">
            ...
            <div class="panel__item comment__panel">
                <div class="block">
                    <div class="title-large fw-black">
                        ...
                        $productCmt = $newCommentModel->getProductCommentByProductId($productId);
                        $i = 0;
                    ...
                    if (isset($_SESSION['userLogin'])) {
                        if (isset($_GET['editCmt'])) {
                            ...
                            $editCmtById = $newCommentModel->getCommentById($getIDCmt);
                            $imgImtById = $newCommentModel->getImgCommentById($_GET['editCmt']);
                            $getIDProduct = $_GET['idProduct'];
                            if ($editCmtById['id_user'] == $_SESSION['userLogin']['id_user']) {
                    ?>
                                <form enctype="multipart/form-data" action="/editComment?idProduct=<?= $getIDProduct ?>&editCmt=<?= $getIDCmt ?>" class="form comment__form" method="post">
                                        ...
                                        <div class="comment_media">
                                            ...
                                            ?>
                                                <div class="comment_media_item">
                                                    <img src="./public/upload/users/<?php echo $item['src'] ?>" alt="">
                                                </div>
                                        <?php
                                            }
                                            echo "</div>";
                            ...
                            ?>
                            <form enctype="multipart/form-data" action="/uploadComment?idProduct=<?= $productId ?>" class="form comment__form" method="post">
                                <input type="file" name="file[]" id="file-1" class="inputfile inputfile-1" data-multiple-caption="{count} đã chọn" multiple />
                                <label for="file-1"><svg xmlns="http://www.w3.org/2000/svg" width="20" height="17" viewBox="0 0 20 17">
                                        <path d="M10 0l-5.2 4.9h3.3v5.1h3.8v-5.1h3.3l-5.2-4.9zm9.3 11.5l-3.2-2.1h-2l3.4 2.6h-3.5c-.1 0-.2.1-.2.1l-.8 2.3h-6l-.8-2.2c-.1-.1-.1-.2-.2-.2h-3.6l3.4-2.6h-2l-3.2 2.1c-.4.3-.7 1-.6 1.5l.6 3.1c.1.5.7.9 1.2.9h16.3c.6 0 1.1-.4 1.3-.9l.6-3.1c.1-.5-.2-1.2-.7-1.5z" />
                                    </svg> <span>Chọn ảnh&hellip;</span></label>
                                ...
                                <div class="flex" style="align-items: center;">
                                    <input type="text" name="inputComment" id="inputComment" class="form__input comment__input" placeholder="Comment">
                                    <button name="submitComment" value="submitComment" type="submit" class="icon-btn send-comment__btn" id="submitComment"><i class="fal fa-paper-plane"></i></button>
                                </div>
                            </form>
                    <?php
                        }
                ...
                <div class="mt30 comment__wrapper">
                    ...
                    $user = new user();
                    $product = new product();
                    $productCmt = $comment->getProductCommentByProductId($productId);
                    foreach ($productCmt as $item) {
                        $getUserByID = $user->getUserById($item['id_user']);
                        $idCmt = $item['id'];
                        $productImgCmt = $comment->getImgCommentById($idCmt);
                        if ($item['is_appear'] == 1) {
                    ?>
                            <div class="comment__item mb30 p30 box-shadow1" style="position: relative;">
                                <div class="flex comment__user">
                                    <div class="flex g12">
                                        <?php
                                        if ($getUserByID['img'] == NULL) {
                                        ?>
                                            <div class="user__avt avt"><img src="./public/upload/users/profile.jpg" alt="user 1" title="<?= $getUserByID['fullname'] ?>" class="imgcover"></div>
                                        <?php
                                        } else {
                                        ?>
                                            <div class="user__avt avt"><img src="./public/upload/users/<?php echo $getUserByID['img'] ?>" alt="user 1" class="imgcover"></div>
                                        <?php
                                        }
                                        ?>
                                        <div class="flex-column flex-between">
                                                ...
                                                <li class="liDadMoreFeatureComment">
                                                    ...
                                                    <div class="divMoreFeatureComment box-shadow1">
                                                        <ul class="ulSonMoreFeatureComment p10">
                                                            ...
                                                            ?>
                                                                <li class="liSonMoreFeatureComment"><a href="/productDetail?idProduct=<?php echo $productId ?>&editCmt=<?php echo $item['id'] ?>">Chỉnh
                                                                        Sửa</a></li>
                                                            ...
                                                            ?>
                                                                <li class="liSonMoreFeatureComment"><a href="/reportComment?reportId=<?php echo $item['id'] ?>&reported=<?php echo $item['reported'] ?>&idProduct=<?php echo $productId ?>">Tố
                                                                        Cáo</a></li>
                                                            <?php
                                                            }
                                                            ?>
                                ...
                                <div class="comment_media">
                                    ...
                                    ?>
                                            <div class="comment_media_item">
                                                <img src="./public/upload/users/<?php echo $result['src'] ?>" alt="">
                                            </div>
                                    <?php
                                        }
                                    }
                                ...
                                <div class="poup_item-bg" style="display: none;">
                                    <div class="poup_item">
                                        ...
                                        <a class="popup_item_delete" href="?mod=page&act=delCmt&reportId=<?php echo $item['id'] ?>&idProduct=<?php echo $productId ?>"><button>Xóa</button></a>
                        ...
                        ?>
                            <div class=" comment__item mb30 p30">
                                <div class="flex g12 comment__user comment__hidden">
                                    <div class="user__avt avt"><img src="./public/upload/users/profile.jpg" alt="admin 1" class="imgcover"></div>
                                    <div class="flex-column flex-between">
                                        <div class="user__name title-medium fw-smb">
            ...
            <div class="panel__item">
                ...
                if (isset($_SESSION['userLogin']) && is_array($_SESSION['userLogin'])) {
                    ...
                    $address = new address();
                    $userAddress = $address->get_addressByIdUser($idUser);
