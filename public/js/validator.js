    ...
    formElement.onsubmit = function (e) {
      ...
      options.rules.forEach((rule) => {
        ...
        inputElements.forEach(function (inputElement) {
          ...
          const formGroup = getParentElement(
            inputElement,
    ...
    async function validate(inputElement, rule) {
      ...
      for (let i = 0; i < rules.length; i++) {
        ...
        if (typeof ruleFunction === "function") {
          ...
          ruleFunction instanceof Promise
        ) {
          try {
  ...
  return {
    ...
    test(value) {
      ...
      return regex.test(value)
        ? undefined
        : message || "Thông tin phải là email!";
    },
  };
};
  ...
  return {
    ...
    test(value) {
      return value.length >= min
        ? undefined
        : message || `Mật khẩu phải có ít nhất ${min} kí tự`;
    },
  };
};
  ...
  return {
    ...
    test(value) {
      return value == confirm()
        ? undefined
        : message || `Mật khẩu xác nhận sai! Vui lòng nhập lại.`;
    },
  };
};
Validator.isUsername = (selector, limit, message) => {
  return {
    ...
    test(value) {
      return value < limit
        ? undefined
        : message || `Tên đăng nhập phải có độ dài dưới ${limit} kí tự`;
    },
  };
};
